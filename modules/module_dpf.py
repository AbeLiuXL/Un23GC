...
class DPF_Module(pl.LightningModule):
    def __init__(self,args):
        ...
        if args.pre_dp is not None:
            self.disp_net = load_pre_model(self.disp_net,args.pre_dp,str_h='disp_net.')
            self.pose_net = load_pre_model(self.pose_net, args.pre_dp, str_h='pose_net.')
        if args.pre_flow is not None:
            self.flow_net = load_pre_model(self.flow_net, args.pre_flow, str_h='flow_net.')
        # self.loss_functions = DepthPoseFlowLosses(args)
        self.loss_functions = DepthPoseFlowLosses(args)
    ...
    def forward(self, batch):
        ...
        flows, flows_inv = self.compute_flow(self.flow_net,imgs)
        DP_photomatric_loss, F_photomatric_loss, \
        D_consistancy_loss, P_consistancy_loss, F_consistancy_loss, DPF_consistancy_loss, \
        D_smooth_loss, F_smooth_loss, PF_trifocal_loss = self.loss_functions(imgs, intrinsics, depths, poses, poses_inv, flows, flows_inv)
        loss =  DP_photomatric_loss * self.args.dppw \
                + F_photomatric_loss * self.args.fpw \
                + D_consistancy_loss * self.args.dcw \
                + P_consistancy_loss * self.args.pcw \
                + F_consistancy_loss * self.args.fcw \
                + DPF_consistancy_loss * self.args.dpfcw \
                + PF_trifocal_loss * self.args.pftw \
                + D_smooth_loss * self.args.dsw \
                + F_smooth_loss * self.args.fsw
        return loss, DP_photomatric_loss, F_photomatric_loss, \
        D_consistancy_loss, P_consistancy_loss, F_consistancy_loss, DPF_consistancy_loss, \
        D_smooth_loss, F_smooth_loss, PF_trifocal_loss

    def training_step(self, batch, batch_nb):
        loss, DP_photomatric_loss, F_photomatric_loss, \
        D_consistancy_loss, P_consistancy_loss, F_consistancy_loss, DPF_consistancy_loss, \
        D_smooth_loss, F_smooth_loss, PF_trifocal_loss = self.forward(batch)
        self.log("T_DP_photomatric_loss", DP_photomatric_loss.item())
        self.log("T_F_photomatric_loss", F_photomatric_loss.item())
        self.log("T_D_consistancy_loss", D_consistancy_loss.item())
        self.log("T_P_consistancy_loss", P_consistancy_loss.item())
        self.log("T_F_consistancy_loss", F_consistancy_loss.item())
        self.log("T_DPF_consistancy_loss", DPF_consistancy_loss.item())
        self.log("T_D_smooth_loss", D_smooth_loss.item())
        self.log("T_F_smooth_loss", F_smooth_loss.item())
        self.log("T_PF_trifocal_loss", PF_trifocal_loss.item())
    ...
    def validation_step(self, batch, batch_nb):
        ...
        D_smooth_loss, F_smooth_loss, PF_trifocal_loss = self.forward(batch)
            ...
            self.log("V_F_photomatric_loss", F_photomatric_loss.item(), sync_dist=True, prog_bar=True, on_epoch=True)
            self.log("V_F_consistancy_loss", F_consistancy_loss.item(), sync_dist=True, on_epoch=True)
            self.log("V_DPF_consistancy_loss", DPF_consistancy_loss.item(), sync_dist=True, on_epoch=True)
            self.log("V_F_smooth_loss", F_smooth_loss.item(), sync_dist=True, on_epoch=True)
            self.log("V_PF_trifocal_loss", PF_trifocal_loss.item(), sync_dist=True, on_epoch=True)
