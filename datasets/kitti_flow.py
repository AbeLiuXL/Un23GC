...
from utils.flowutils import flow_io
import torch
import os
...
class KittiFlow2015(data.Dataset):
    def __init__(self,root,phase='training',transform=None, occ='flow_occ',N=200):
        self.root = Path(root)
        self.N = N
    ...
    def __getitem__(self, index):
        ...
        ref_img_path = self.root.joinpath('data_scene_flow_multiview', self.phase, 'image_2',str(index).zfill(6) + '_11.png')
       ...
       # print(gtFlow_occ.shape)
        ...
        return tgt_img, ref_img, intrinsics, np.linalg.inv(intrinsics), gtFlow_occ,noc_mask.astype(float), obj_map


class KittiFlow2012(data.Dataset):
    def __init__(self,root,phase='training',transform=None,N=194):
        self.root = Path(root)
        self.N = N
        self.transform = transform
        self.phase = phase
    def __len__(self):
        return self.N
    def __getitem__(self, index):
        tgt_img_path =  self.root.joinpath('data_stereo_flow_multiview', self.phase, 'image_2',str(index).zfill(6)+'_10.png')
        ref_img_path = self.root.joinpath('data_stereo_flow_multiview', self.phase, 'image_2',str(index).zfill(6) + '_11.png')
        gt_flow_path_occ = self.root.joinpath('data_stereo_flow', self.phase, 'flow_occ', str(index).zfill(6) + '_10.png')
        gt_flow_path_noc = self.root.joinpath('data_stereo_flow', self.phase, 'flow_noc', str(index).zfill(6) + '_10.png')
        #cam_calib_path = self.root.joinpath('data_stereo_flow_calib', self.phase, 'calib',str(index).zfill(6) + '.txt')
        #obj_map_path = self.root.joinpath('data_scene_flow', self.phase, 'obj_map', str(index).zfill(6) + '_10.png')
        ...
        gtFlow_occ = torch.FloatTensor(gtFlow_occ.transpose(2, 0, 1))
        #noc_mask = torch.FloatTensor(noc_mask)
        intrinsics = np.eye(3) #get_intrinsics(cam_calib_path).astype('float32')
        ...
        if self.transform is not None:
            ...
            intrinsics = np.copy(intrinsics)
        return tgt_img, ref_img, intrinsics, np.linalg.inv(intrinsics), gtFlow_occ,noc_mask.astype(float), 1
...
class ValidationFlowKitti2015_seq(data.Dataset):
    """
        Kitti 2015 flow loader
        transform functions must take in a list a images and a numpy array which can be None
    """
...
if __name__ == '__main__':
    kitti2012 = KittiFlow2012('../../Data/Kitti/kitti_flow2012')
    kitti2015 = KittiFlow2015('../../Data/Kitti/kitti_flow2015')
